...
namespace Avanade.BestPractices.Service
{
    public class RideService : ServiceBase<Ride>, IRideService
    {
        ...
        public Task<Ride> GetByIdAsync(Guid id, int queryContext = default)
        {
            return _rideRepository.GetByIdAsync(id, queryContext);
        }
        ...
        public async Task<Ride> StartAsync(Guid vehicleId, Guid accountId)
        {
            ...
            var account = await _accountService.GetByIdAsync(accountId, AccountQueryContext.GetById.StartRideValidation.ToInt());
            if (account is null)
            ...
            var vehicle = await _vehicleService.GetByIdAsync(vehicleId, VehicleQueryContext.GetById.StartRideValidation.ToInt());
            if (vehicle is null)
